...
class PageController extends BaseAdminController
{
    ...
    public function getEdit(Request $request, Page $object, $id)
    {
        $oldInputs = old();
        if ($oldInputs && $id == 0) {
        ...
        if (!$id == 0) {
            ...
            $this->_setPageTitle('Edit page', $item->title);
    ...
    public function postEdit(Request $request, Page $object, PageMeta $objectMeta, $id)
    {
        $data = $request->all();
        if (!$data['slug']) {
