...
class PostController extends BaseAdminController
{
    ...
    public function getEdit(Request $request, Post $object, $id)
    {
        $oldInputs = old();
        if ($oldInputs && $id == 0) {
        ...
        if (!$id == 0) {
            ...
            $this->_setPageTitle('Edit post', $item->title);
    ...
    public function postEdit(Request $request, Post $object, PostMeta $objectMeta, $id)
    {
        $data = $request->all();
        if (!$data['slug']) {
            $data['slug'] = str_slug($data['title']);
        }
        $data['thumbnail'] = $request->thumbnail_path;
