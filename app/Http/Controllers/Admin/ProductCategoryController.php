...
class ProductCategoryController extends BaseAdminController
{
    ...
    public function getEdit(Request $request, ProductCategory $object, $id)
    {
        $oldInputs = old();
        if ($oldInputs && $id == 0) {
        ...
        if (!$id == 0) {
            ...
            $this->dis['object'] = $item;
            $this->_setPageTitle('Edit product category', $item->title);
    ...
    public function postEdit(Request $request, ProductCategory $object, ProductCategoryMeta $objectMeta, $id)
    {
        $data = $request->all();
        if (!$data['slug']) {
