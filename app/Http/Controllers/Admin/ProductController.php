...
class ProductController extends BaseAdminController
{
    ...
    public function getEdit(Request $request, Product $object, Models\ProductAttributeSet $objectSet, $id)
    {
        $oldInputs = old();
        if ($oldInputs && $id == 0) {
    ...
    public function postEdit(Request $request, Product $object, ProductMeta $objectMeta, $id)
    {
        ...
        if ($id == 0) {
            return redirect()->to(asset($this->adminCpAccess . '/' . $this->routeLink . '/edit/' . $result['object']->product_id));
        }
        return redirect()->back();
    }
